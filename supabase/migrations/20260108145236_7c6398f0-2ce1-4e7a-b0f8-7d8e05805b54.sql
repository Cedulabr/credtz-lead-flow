-- Fix: Update the import_leads_from_csv function to check duplicates by name + phone together
-- (The previous migration kept old logic, this one fixes it)
...
BEGIN
  ...
  LOOP
    ...
    -- Phone is the main differentiator, but we also check name to allow same phone with different people
    ...
    IF existing_lead IS NOT NULL THEN
      ...
      duplicate_details := duplicate_details || jsonb_build_object(
        ...
        'telefone', v_telefone,
        'convenio', v_convenio,
        'motivo', 'Lead já existente na base. Importado anteriormente em ' || to_char(existing_lead.created_at, 'DD/MM/YYYY'),
