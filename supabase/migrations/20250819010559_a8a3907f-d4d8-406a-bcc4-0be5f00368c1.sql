-- CRITICAL SECURITY HARDENING - Phase 1: Data Protection
...
-- 6. ADD COMPREHENSIVE AUDIT TRIGGERS FOR SENSITIVE TABLES
-- Trigger for baseoff access (already in the function above)
DROP TRIGGER IF EXISTS audit_baseoff_access ON baseoff;
CREATE TRIGGER audit_baseoff_access
  AFTER SELECT ON baseoff
  FOR EACH STATEMENT
  EXECUTE FUNCTION public.audit_sensitive_access();

-- Trigger for leads access
...
-- Trigger for clientes access  
DROP TRIGGER IF EXISTS audit_clientes_access ON clientes;
