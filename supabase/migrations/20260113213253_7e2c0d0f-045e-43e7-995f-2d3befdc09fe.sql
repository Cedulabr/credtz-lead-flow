-- 1. Adicionar campo phone2 na tabela leads
ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS phone2 TEXT;

-- 2. Adicionar campo phone2 na tabela leads_database  
ALTER TABLE public.leads_database ADD COLUMN IF NOT EXISTS phone2 TEXT;

-- 3. Criar função para obter DDDs disponíveis para filtro
CREATE OR REPLACE FUNCTION public.get_available_ddds()
...
BEGIN
  RETURN QUERY
  SELECT 
    SUBSTRING(ld.phone FROM 1 FOR 2) as ddd,
    COUNT(*) as available_count
  FROM public.leads_database ld
  WHERE ld.is_available = true
    AND ld.phone IS NOT NULL
    AND LENGTH(ld.phone) >= 10
  GROUP BY SUBSTRING(ld.phone FROM 1 FOR 2)
  ORDER BY ddd;
END;
$$;

-- 4. Atualizar função request_leads_with_credits para aceitar filtro de DDDs
...
BEGIN
  ...
  -- Return leads with optional DDD filter
  RETURN QUERY
  WITH selected_leads AS (
    SELECT ld.id, ld.name, ld.cpf, ld.phone, ld.phone2, ld.convenio, ld.banco
    FROM public.leads_database ld
    WHERE ld.is_available = true
  ...
  WHERE ld.id = sl.id
  RETURNING sl.id, sl.name, sl.cpf, sl.phone, sl.phone2, sl.convenio, sl.banco;
...
-- 5. Atualizar função de importação para aceitar phone2
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      INSERT INTO public.leads_database (name, phone, phone2, convenio, is_available)
      VALUES (
        lead_item->>'nome',
        lead_item->>'telefone',
