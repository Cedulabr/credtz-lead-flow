...
-- Adicionar coluna lead_id na tabela leads_distribution
ALTER TABLE public.leads_distribution 
ADD COLUMN IF NOT EXISTS lead_id uuid REFERENCES public.leads_database(id);

-- Criar índice para melhor performance
CREATE INDEX IF NOT EXISTS idx_leads_distribution_lead_id 
ON public.leads_distribution(lead_id);

-- Atualizar função get_available_convenios para funcionar com ou sem lead_id
...
BEGIN
  ...
  WHERE ld.is_available = true
    -- Excluir leads já distribuídos (verificar tanto por lead_id quanto por id)
    AND NOT EXISTS (
...
-- Atualizar função request_leads_with_credits
...
BEGIN
  ...
  IF actual_leads_count > 0 THEN
    ...
    SELECT tsl.id, tsl.cpf, auth.uid(), now(), now() + interval '10 years'
