-- Update the import_leads_from_csv function to pad CPF with leading zeros
...
BEGIN
  ...
  LOOP
    ...
    -- Check for duplicates (CPF or phone already exists)
    SELECT * INTO existing_lead
    FROM leads_database
    WHERE cpf = v_cpf OR phone = v_telefone
    LIMIT 1;
    ...
    IF existing_lead IS NOT NULL THEN
      ...
      duplicate_details := duplicate_details || jsonb_build_object(
        ...
        'convenio', v_convenio,
        'motivo', CASE 
          WHEN existing_lead.cpf = v_cpf AND existing_lead.phone = v_telefone THEN 'CPF e Telefone já existem'
          WHEN existing_lead.cpf = v_cpf THEN 'CPF já existe'
