...
type PeopleSection = 'menu' | 'users' | 'companies' | 'indications';
...
export function AdminPeople() {
  const [activeSection, setActiveSection] = useState<PeopleSection>('menu');

  const sections: SectionItem[] = [
    {
      id: 'users',
      label: 'Usuários',
      description: 'Gerenciar usuários, permissões e status',
      icon: Users,
      color: 'text-blue-600',
    ...
    },
    {
      id: 'companies',
      label: 'Empresas',
      description: 'Gerenciar empresas e suas configurações',
      icon: Building2,
    ...
    },
    {
      id: 'indications',
      label: 'Indicações',
      description: 'Gerenciar indicações de clientes',
      icon: UserPlus,
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'indications':
        return <AdminIndicationsManagement />;
      default:
        return null;
    }
  };
  ...
  if (activeSection === 'menu') {
    return (
      <div className="space-y-6 px-4 md:px-0">
        <div>
          <h1 className="text-xl md:text-2xl font-bold">Pessoas</h1>
          <p className="text-sm text-muted-foreground">
            Gerencie usuários, empresas e indicações
          </p>
        </div>
