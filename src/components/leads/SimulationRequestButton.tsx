...
interface SimulationRequestButtonProps {
  leadId: string;
  leadName: string;
...
export function SimulationRequestButton({
  leadId,
  leadName,
  currentSimulationStatus,
  onSuccess,
  variant = "icon",
}: SimulationRequestButtonProps) {
  const { toast } = useToast();
  const { requestSimulation } = useSimulationNotifications();
  const [isOpen, setIsOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [isSuccess, setIsSuccess] = useState(false);

  const hasActiveSimulation = currentSimulationStatus && 
    ['solicitada', 'em_andamento', 'concluida', 'enviada'].includes(currentSimulationStatus);

  const getStatusInfo = () => {
    switch (currentSimulationStatus) {
      case 'solicitada':
        return { label: 'Aguardando', color: 'text-amber-600', bgColor: 'bg-amber-50' };
      case 'em_andamento':
        return { label: 'Em Andamento', color: 'text-blue-600', bgColor: 'bg-blue-50' };
      case 'concluida':
      case 'enviada':
        return { label: 'Pronta!', color: 'text-emerald-600', bgColor: 'bg-emerald-50' };
      case 'recebida':
        return { label: 'Confirmada', color: 'text-green-600', bgColor: 'bg-green-50' };
      default:
        return null;
    }
  };
  ...
  if (hasActiveSimulation && statusInfo) {
    return (
      ...
      >
        ...
        {(currentSimulationStatus === 'concluida' || currentSimulationStatus === 'enviada') && (
          ...
          <AnimatePresence mode="wait">
            {isSuccess ? (
              ...
              >
                ...
                <p className="text-lg font-semibold text-emerald-700">Solicitação Enviada!</p>
                <p className="text-sm text-gray-500 mt-1">O gestor será notificado</p>
              </motion.div>
            ) : (
              <motion.div
                initial={{ opacity: 0 }}
