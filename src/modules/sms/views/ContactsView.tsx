...
export const ContactsView = ({ contactLists, onRefresh }: ContactsViewProps) => {
  ...
  const handleCsvImport = async (listId: string) => {
    ...
    input.onchange = async (e) => {
      ...
      for (const line of lines.slice(1)) { const cols = line.split(/[,;]/).map((c) => c.trim().replace(/"/g, "")); const phone = (cols[1] || cols[0] || "").replace(/\D/g, ""); if (phone.length >= 10) contacts.push({ list_id: listId, name: cols.length > 1 ? cols[0] : null, phone, source: "csv" }); }
      if (contacts.length === 0) { toast.error("Nenhum telefone válido no CSV"); return; }
      const { error } = await supabase.from("sms_contacts").insert(contacts as any);
      if (error) toast.error("Erro ao importar"); else { toast.success(`${contacts.length} contatos importados`); onRefresh(); if (expandedList === listId) toggleExpand(listId); }
    };
    input.click();
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setCreateOpen(true)} className="gap-2"><Plus className="h-4 w-4" /> Nova Lista</Button>
      </div>
            ...
            <div key={list.id} className="rounded-xl border border-border/50 bg-card overflow-hidden hover:border-border transition-colors">
              <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-muted/20 transition-colors" onClick={() => toggleExpand(list.id)}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); setAddPhoneListId(list.id); setAddPhoneOpen(true); }}><Phone className="h-3.5 w-3.5" /></Button>
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); handleCsvImport(list.id); }}><Upload className="h-3.5 w-3.5" /></Button>
